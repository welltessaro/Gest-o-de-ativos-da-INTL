          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              <button 
                ...
                className="relative p-3 rounded-2xl bg-slate-50 border border-slate-100 text-slate-800 hover:bg-slate-100 transition-all"
              >
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-3 w-80 bg-white rounded-[2rem] shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-top-2 z-[100]">
                  <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                    <span className="text-xs font-black uppercase text-slate-900 tracking-widest">Notificações</span>
                  ...
                  <div className="max-h-96 overflow-y-auto p-4 space-y-3">
                    {todayDeliveries.map((d, i) => (
                      <div key={i} className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100">
                         <p className="text-xs font-bold text-slate-900">{d.item} Chegando!</p>
                         <p className="text-[10px] text-slate-700 font-medium">Protocolo: {d.id}</p>
                      </div>
                    ))}
                    {todayDeliveries.length === 0 && <p className="text-center py-6 text-slate-600 font-medium">Sem avisos hoje.</p>}
                  </div>
                </div>
              )}
            </div>
