...
import { Asset, EquipmentRequest, Employee, AssetTypeConfig, AccountingAccount, Department } from '../types';
  ...
  const chartData = useMemo(() => {
    return assetTypeConfigs.map(config => {
      ...
      return {
        name: config.name,
        qty: count,
        classificationName: account?.name || 'Sem Classificação'
      };
    })
    .filter(item => item.qty > 0)
    .sort((a, b) => b.qty - a.qty);
  }, [activeAssets, assetTypeConfigs, accounts]);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-300">
              ...
              <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-white">
                ...
                {/* ... (Resto do conteúdo do modal igual) ... */}
                ...
                {/* ... Outros casos (maintenance, retired, value) permanecem iguais ... */}
