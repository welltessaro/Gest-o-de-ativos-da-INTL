...
import { Building2, Plus, Search, Trash2, Edit2, Package, Calendar, ShieldCheck, X, ChevronRight } from 'lucide-react';
...
const CompanyManager: React.FC<CompanyManagerProps> = ({ companies, assets, onAddCompany, onUpdateCompany, onRemoveCompany }) => {
  ...
  const getAssetCount = (companyId: string) => assets.filter(a => a.companyId === companyId).length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-2">
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    if(confirm(`Deseja realmente remover a unidade ${company.name}?`)) {
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <h4 className="text-xl font-black text-slate-800 tracking-tight group-hover:text-blue-600 transition-colors">{company.name}</h4>
                <Edit2 className="w-3.5 h-3.5 text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{company.cnpj || 'CNPJ não informado'}</p>
            </div>

            <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Package className="w-4 h-4 text-slate-300" />
                <span className="text-sm font-black text-slate-700">{getAssetCount(company.id)} Ativos</span>
              </div>
            ...
            <div className="absolute top-[-20%] right-[-10%] w-32 h-32 bg-blue-500/5 rounded-full blur-3xl pointer-events-none group-hover:bg-blue-500/10 transition-colors" />
          </div>
        ))}
        ...
        {filteredCompanies.length === 0 && (
          <div className="col-span-full py-24 text-center flex flex-col items-center justify-center bg-white rounded-[3rem] border border-dashed border-slate-200">
             ...
             <p className="text-slate-400 font-black uppercase tracking-widest text-xs">Nenhuma empresa ou unidade encontrada</p>
             <button onClick={handleOpenCreate} className="mt-4 text-blue-600 font-bold hover:underline">Cadastrar primeira unidade</button>
          </div>
        )}
      </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className={`p-8 ${editingCompany ? 'bg-amber-600' : 'bg-blue-600'} text-white flex justify-between items-center transition-colors`}>
               <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center">
                    {editingCompany ? <Edit2 className="w-6 h-6" /> : <Building2 className="w-6 h-6" />}
                  </div>
                  <div>
                    <h3 className="text-2xl font-black tracking-tight">
                      {editingCompany ? 'Editar Empresa' : 'Nova Empresa'}
                    </h3>
                    <p className="text-[10px] font-black uppercase tracking-widest text-white/70">Módulo de Gestão de Unidades</p>
                  </div>
               </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-6">
               <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nome da Empresa / Unidade</label>
                  <input 
               ...
               <div className={`p-6 rounded-3xl flex items-start gap-4 border transition-all ${editingCompany ? 'bg-amber-50 border-amber-100 text-amber-700' : 'bg-blue-50 border-blue-100 text-blue-700'}`}>
                  ...
                  <div className="space-y-1">
                    <p className="text-xs font-black uppercase tracking-tight">Informação do Sistema</p>
                    <p className="text-[11px] font-bold leading-relaxed opacity-80">
                      {editingCompany 
                        ? 'As alterações feitas aqui serão propagadas para todos os ativos e colaboradores vinculados a esta unidade.'
                        : 'Cadastre a unidade para começar a vincular ativos e colaboradores específicos deste local.'}
                    </p>
                  </div>
               </div>
               ...
               <div className="flex gap-3">
                 ...
                 >
                    {editingCompany ? 'Salvar Alterações' : 'Cadastrar Unidade'}
                 </button>
               </div>
            </form>
          </div>
